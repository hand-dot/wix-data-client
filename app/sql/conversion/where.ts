...
import { convertValueExp } from './general';
import type { ConvertionObj } from '../types';


export function convertCondExp(convertionObj: ConvertionObj, parsedInputArr: string[], index: number) {
...
function convertLikeValueExp(columnName: string, condValue: string, convertionObj: any) {
    let condObj: {
        [x: string]: {
            [x: string]: string;
        };
...
function checkIfAnotherCondExp(parsedInputArr: string[], index: number) {
...
function convertCondOperator(operator: string) {
