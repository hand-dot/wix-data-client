...
export function convertCollectionName(convertionObj: ConvertionObj, parsedInputArr: string[], index: number) {
...
export function convertValueExp(parsedInputArr: string[], index: number) {
    ...
    if (value[0] === "'" || value[0] === '"') {
        ...
        if (value.length === 1 || (value.slice(-1) === ',' && value.length >= 2 && value.slice(-2) !== terminationChar + ',') || (value.slice(-1) !== ',' && value.slice(-1) !== terminationChar)) {
