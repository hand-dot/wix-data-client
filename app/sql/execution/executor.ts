...
import { buildWixDataQuery, addOrderByToQuery, addLimitToQuery, buildWixDataAggregateQuery, buildWixDataFilter } from './query-builder';
import { buildTableOfFunctionsResults, buildTableFromJoinResults, buildTableFromResults } from './results-builder';
import type { ConvertionObj } from '../types';

export async function execute(convertionObj: ConvertionObj) {
    ...
    switch (convertionObj.expType) {
        ...
        case 'DELETE': {
            wixDataFunc = buildWixDataQuery(convertionObj.collectionName, convertionObj.filterObj, convertionObj.anotherCondType);
            funcResults = await wixDataFunc.find();
            funcResults = funcResults.items.map((item: any) => item._id);
        ...
        case 'LEFT JOIN': {
            ...
            joinedWixDataFunc = buildWixDataQuery(convertionObj.joinedCollectionName, convertionObj.condsMap[convertionObj.joinedCollectionName], convertionObj.anotherCondType);
...
function adjustConvertionObjectToJoin(convertionObj: ConvertionObj) {
